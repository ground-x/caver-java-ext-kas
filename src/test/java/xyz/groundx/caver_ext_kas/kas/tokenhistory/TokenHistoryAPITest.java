...
public class TokenHistoryAPITest {
    ...
    @Test
    public void getFTSummaryByAddress() throws ApiException {
        PageableAccountFT accountFT = caver.kas.tokenHistory.getFTSummaryByAddress(account);
        assertNotNull(accountFT);
    }

    @Test
    public void getFTSummaryByAddressWithSize() throws ApiException {
        TokenHistoryQueryOptions options = new TokenHistoryQueryOptions();
        options.setSize(1L);

        PageableAccountFT accountFT = caver.kas.tokenHistory.getFTSummaryByAddress(account, options);
        assertEquals(1, accountFT.getItems().size());
        assertNotNull(accountFT);
    }

    @Test
    public void getFTSummaryByAddressWithCursor() throws ApiException {
        TokenHistoryQueryOptions options = new TokenHistoryQueryOptions();
        options.setSize(1L);

        PageableAccountFT accountFT = caver.kas.tokenHistory.getFTSummaryByAddress(account, options);
        options.setCursor(accountFT.getCursor());

        PageableAccountFT result = caver.kas.tokenHistory.getFTSummaryByAddress(account, options);
        assertEquals(1, accountFT.getItems().size());
        assertNotNull(accountFT);
    }

    @Test
    public void getFTSummaryByAddressWithCAFilter() throws ApiException {
        PageableAccountFT accountFT = caver.kas.tokenHistory.getFTSummaryByAddress(account);
        TokenHistoryQueryOptions options = new TokenHistoryQueryOptions();
        options.setCaFilter(accountFT.getItems().get(0).getAddress());
    ...
    @Test
    public void getFTSummaryByAddressAsync() throws ApiException, ExecutionException, InterruptedException {
        CompletableFuture<PageableAccountFT> future = new CompletableFuture<>();
        Call result = caver.kas.tokenHistory.getFTSummaryByAddressAsync(account, new ApiCallback<PageableAccountFT>() {
            @Override
            public void onFailure(ApiException e, int statusCode, Map<String, List<String>> responseHeaders) {
                future.completeExceptionally(e);
            }
            ...
            @Override
            public void onSuccess(PageableAccountFT result, int statusCode, Map<String, List<String>> responseHeaders) {
                future.complete(result);
            }
